...
Constructs an Neural network model that predicts the RBP activity from BoW vectors containing 
...
for i in range(2):
    ...
    for k in range(4,9):
        ...
        hDim = int(inDim/32)    # Number of hidden nodes
        outDim = 1  # Number of output nodes
        
        # Creates the neural network model and specifies the hidden layer to use the sigmoid function
        model = torch.nn.Sequential(
        torch.nn.Linear(inDim, hDim),
        torch.nn.Sigmoid(),
        torch.nn.Linear(hDim, outDim))
        model.cuda()    # Puts the model into the CUDA
